...
local UserInputService = game:GetService("UserInputService") -- NOVO: Detecta teclas
...
local menuVisivel = false -- Controla se o menu aparece
...
-- [ TOGGLE COM SHIFT ESQUERDO ]
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    if gameProcessed then return end -- Evita fechar se estiver escrevendo no chat
    if input.KeyCode == Enum.KeyCode.LeftShift then
        if MenuFrame.Visible then
            MenuFrame.Visible = false
        elseif currentKey ~= "" or player.UserId == OWNER_ID then
...
Botao.MouseButton1Click:Connect(function()
    ...
    if not key:match("^DVZ%-") then Status.Text = "❌ Key deve começar com DVZ-" return end
...
function iniciarTimer(segundos)
    spawn(function()
        local tempo = segundos
        while tempo > 0 do
            local h, m, s = math.floor(tempo/3600), math.floor((tempo%3600)/60), tempo%60
            TimerLabel.Text = string.format("⏱️ %02d:%02d:%02d", h, m, s)
            wait(1)
            tempo = tempo - 1
        end
        if player.UserId ~= OWNER_ID then
            player:Kick("Sua key expirou!")
        end
    end)
end
...
function iniciarSistema()
    ...
    RunService.RenderStepped:Connect(function()
            ...
            for _, alvo in pairs(Players:GetPlayers()) do
                if alvo ~= player and alvo.Character and alvo.Character:FindFirstChild("Head") then
                    ...
                    if vis then
                        ...
                        box.Size, box.Position = UDim2.new(0, 100, 0, 20), UDim2.new(0, pos.X - 50, 0, pos.Y - 10)
                        box.BackgroundTransparency, box.Text, box.TextColor3 = 1, alvo.Name, Color3.fromRGB(255, 0, 0)
        ...
        if getAimbot() or getAimLock() then
            local maisProximo, menorDist = nil, 250
            for _, alvo in pairs(Players:GetPlayers()) do
                if alvo ~= player and alvo.Character and alvo.Character:FindFirstChild("Head") then
                    local head = alvo.Character.Head
                    local pos = camera:WorldToViewportPoint(head.Position)
                    local dist = (Vector2.new(pos.X, pos.Y) - Vector2.new(camera.ViewportSize.X/2, camera.ViewportSize.Y/2)).Magnitude
                    if dist < menorDist then menorDist, maisProximo = dist, head end
                end
            end
...
if player.UserId == OWNER_ID then
    ...
    KeyDisplay.Text, TimerLabel.Text = "ACESSO: DONO (PERM)", "⏱️ INFINITO"
    TimerLabel.TextColor3 = Color3.fromRGB(255, 215, 0)
    currentKey = "OWNER"
    iniciarSistema()
else
    print("✅ DVZ Script carregado | By Davz")
end
